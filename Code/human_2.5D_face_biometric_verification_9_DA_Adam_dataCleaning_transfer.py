...
all_images_cropped = '../../UND 2.5D cropped face data/2.5D Cropped Data/*.jpg'
dataAugmentation_crop = '../../UND 2.5D cropped face data/2.5D_cropped_data_DA/*_cropped.jpg'
dataAugmentation_rotate = '../../UND 2.5D cropped face data/2.5D_cropped_data_DA/*_rotated*.jpg'
noise_augmentation = '../../UND 2.5D cropped face data/2.5D_cropped_data_DA/*_noise.jpg'
#categories_n = 200
...
noise_images = []
noise_targets = []
noise_images, noise_targets, noise_labels = load_dataset(
...
target_numbers=np.array(targets_array[:,0],dtype=np.int32)
for i in range(np.max(target_numbers),-1,-1):
    subject_list=list(np.where(target_numbers == i))
    if (len(subject_list[0])==7):
        for index in sorted(subject_list[0], reverse=True):
            del images[index]
            del targets[index]
        target_numbers=np.delete(target_numbers,subject_list[0])
        #labels=np.delete(labels,i)

#noise_images=np.array(noise_images)
noise_targets_array = np.array(noise_targets)
#noise_labels = np.array(noise_labels)
noise_target_numbers=np.array(noise_targets_array[:,0],dtype=np.int32)
for i in range(np.max(noise_target_numbers),-1,-1):
    subject_list=list(np.where(noise_target_numbers == i))
    if (len(subject_list[0])==1):
        for index in sorted(subject_list[0], reverse=True):
            del noise_images[index]
            del noise_targets[index]
        noise_target_numbers=np.delete(noise_target_numbers,subject_list[0])
        #noise_labels=np.delete(noise_labels,i)

images_num = len(images)
categories_n=len(np.unique(noise_target_numbers))
...
x_train, x_test, y_train, y_test = train_test_split(
    gray_images, targets, test_size=0.3, shuffle=True, random_state=mystate)
x_val, x_test, y_val, y_test = train_test_split(
    x_test, y_test, test_size=0.5, shuffle=True, random_state=mystate)
...
pairTrain, labelTrain, nameTrain = make_pairs(x_train, y_train)
print("Validation pairs:")
pairVal, labelVal, nameVal = make_pairs(x_val, y_val)
print("Testing pairs:")
pairTest, labelTest, nameTest = make_pairs(x_test, y_test)
...
def train_model(datax, datay, valx, valy,n_fixed):
    ...
    model.compile(optimizer=opt, loss='binary_crossentropy',
                        metrics=['accuracy'])
    model.summary()
    keras.utils.plot_model(model,"../../Outputs/verification_2.5D_model_9_DA_transfer.png",show_shapes=True)
    # fit model
...
history, model = train_model(pairTrain, labelTrain, pairVal, labelVal,0)
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/verification_2.5D_model9_DA_transfer_graph.png')
plt.close()
model.save('../Models/human_2.5D_verification_model9_DA_transfer.h5')
print("saved")
...
plt.title('ROC curve on UND 2.5D Dataset')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.grid()
plt.legend(['No skill', 'ROC Curve'], loc='lower right')
plt.savefig('../../Outputs/verification_2.5D_model9_DA_transfer_roc.png')
plt.close()
roc_display = RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=auc)
roc_display.plot()
plt.savefig('../../Outputs/verification_2.5D_model9_DA_transfer_display_roc.png')
plt.close()
fnr = 1 - tpr
...
plt.savefig('../../Outputs/verification_2.5D_model9_DA_transfer_confusionMatrix.png')
plt.close()
...
legend = ax.legend()
plt.savefig('../../Outputs/verification_2.5D_model9_DA_transfer_EER.png')
plt.close()


pred_prob=pred_prob*100
same=[pred_class==labelTest]
A=np.array(list(zip(nameTest,pred_prob,pred_class,labelTest,same)))
print(A)

np.savetxt('../../Outputs/verification_2.5D_model9_DA_transfer_scores.csv', A, fmt='%s', delimiter=',', header="Image1,Image2,pred probability,pred class, actual class, same(0)")