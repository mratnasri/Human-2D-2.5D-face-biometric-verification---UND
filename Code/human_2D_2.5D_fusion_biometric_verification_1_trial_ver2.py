...
x_train_2D, x_test_2D, y_train_2D, y_test_2D = train_test_split(
    gray_images_2D, targets_2D, test_size=0.3, shuffle=True, stratify=targets_2D, random_state=mystate)
x_val_2D, x_test_2D, y_val_2D, y_test_2D = train_test_split(
    x_test_2D, y_test_2D, test_size=0.5, shuffle=True, stratify=y_test_2D, random_state=mystate)

x_train_25D, x_test_25D, y_train_25D, y_test_25D = train_test_split(
    gray_images_25D, targets_25D, test_size=0.3, shuffle=True, stratify=targets_25D, random_state=mystate)
x_val_25D, x_test_25D, y_val_25D, y_test_25D = train_test_split(
    x_test_25D, y_test_25D, test_size=0.5, shuffle=True, stratify=y_test_25D, random_state=mystate)
...
def make_pairs(images_2D,images_25D, labels):
    pairImages = []
    pairLabels = []
    # print(labels)
...
pairTrain, labelTrain = make_pairs(x_train_2D, x_train_25D, y_train_2D)
print("Validation pairs:")
pairVal, labelVal = make_pairs(x_val_2D, x_val_25D, y_val_2D)
print("Testing pairs:")
pairTest, labelTest = make_pairs(x_test_2D, x_test_25D, y_test_2D)


"""def model_config(input_shape):
    input_2D_left = Input(shape=input_shape,name="input_2D_left")
    input_2D_right = Input(shape=input_shape,name="input_2D_right")
    input_25D_left = Input(shape=input_shape,name="input_25D_left")
    input_25D_right = Input(shape=input_shape,name="input_25D_right")

    model_2D=load_model('../Models/human_2D_verification_model3_DA_stratify.h5')
    model_2D._name="model_2D"
    model_25D=load_model('../Models/human_2.5D_verification_model3_DA_stratify.h5')
    ...
    merged = keras.layers.Concatenate(axis=1)([encoded_2D,encoded_25D])
    #print(merged.shape)
    # Add a dense layer with a sigmoid unit to generate the similarity score
    prediction = Dense(1, activation='sigmoid')(merged)

    # Connect the inputs with the outputs
    fusion_net = Model(inputs=[input_2D_left, input_2D_right, input_25D_left, input_25D_right], outputs=prediction, name='fusion_model_2D-2.5D')
...
def train_model(datax, datay, valx, valy):
    model_2D=load_model('../Models/human_2D_verification_model1.h5')
    model_2D._name="model_2D"
    model_25D=load_model('../Models/human_2.5D_verification_model1.h5')
    model_25D._name="model_2.5D"
    ...
    model.compile(optimizer=opt, loss='binary_crossentropy',
                        metrics=['accuracy'])
    model.summary()
    #keras.utils.plot_model(model,"../../Outputs/fusion_2D-2.5D_model_2_ResNet34_model5_ver2.png",show_shapes=True)
    # fit model
    history = model.fit([datax[:, 0], datax[:, 1],datax[:,2], datax[:,3]], datay[:], epochs=1, batch_size=32,
...
model.save('../Models/human_2D_2.5D_fusion_verification_model1_trial_ver2.h5')
print("saved")
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/verification_2D_2.5D_fusion_model1_graph_2.png')
plt.close()
...
plt.savefig('../../Outputs/verification_2D_2.5D_fusion_model1_roc_2.png')
plt.close()
roc_display = RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=auc)
roc_display.plot()
plt.savefig('../../Outputs/verification_2D_2.5D_fusion_model1_display_roc_2.png')
plt.close()
fnr = 1 - tpr
...
plt.savefig('../../Outputs/verification_2D_2.5D_fusion_model1_2.png')
plt.close()
...
legend = ax.legend()
plt.savefig('../../Outputs/verification_2D_2.5D_fusion_model1_EER_2.png')
plt.close()'''
