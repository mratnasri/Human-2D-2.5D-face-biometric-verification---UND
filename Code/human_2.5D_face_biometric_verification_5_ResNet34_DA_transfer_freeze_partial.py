...
x_train, x_test, y_train, y_test = train_test_split(
    images, targets, test_size=0.3, shuffle=True, random_state=mystate)
...
'''def tf_siamese_nn(shape, embedding=64, fineTune=False):
    ...
    base_model = ResNet34(input_shape=shape, weights='imagenet', include_top=False)
    ...
    outputs = keras.layers.Dense(embedding)(x)
    model = keras.Model(inputs, outputs)

    return model


model = tf_siamese_nn(IMG_SHAPE, 1000, False)
model.summary()
keras.utils.plot_model(model,"../../Outputs/verification_2D_model_4_ResNet34_DA_stratify_featureExtractor.png",show_shapes=True)
...
def model_config(input_shape):
    ...
    model = tf_siamese_nn(input_shape)
...
def train_model(datax, datay, valx, valy,n_fixed):
    ...
    model=load_model('../Models/human_2D_verification_model5_ResNet34_DA.h5')
    ...
    model.compile(optimizer=opt, loss='binary_crossentropy',
                        metrics=['accuracy'])
    model.summary()
    keras.utils.plot_model(model,"../../Outputs/verification_2.5D_model5_ResNet34_DA_transfer_freeze_partial.png",show_shapes=True)
    # fit model
...
history, model = train_model(pairTrain, labelTrain, pairVal, labelVal,3)
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/verification_2.5D_model5_ResNet34_DA_transfer_freeze_partial_graph.png')
plt.close()
model.save('../Models/human_2.5D_verification_model5_ResNet34_DA_transfer_freeze_partial.h5')
print("saved")
...
plt.savefig('../../Outputs/verification_2.5D_model5_ResNet34_DA_transfer_freeze_partial_roc.png')
plt.close()
roc_display = RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=auc)
roc_display.plot()
plt.savefig('../../Outputs/verification_2.5D_model5_ResNet34_DA_transfer_freeze_partial_display_roc.png')
plt.close()
fnr = 1 - tpr
...
plt.savefig('../../Outputs/verification_2.5D_model5_ResNet34_DA_transfer_freeze_partial_confusionMatrix.png')
plt.close()
...
legend = ax.legend()
plt.savefig('../../Outputs/verification_2.5D_model5_ResNet34_DA_transfer_freeze_partial_EER.png')
plt.close()