...
model.summary()
keras.utils.plot_model(model,"../../Outputs/verification_2D_model5_ResNet34_DA_featureExtractor.png",show_shapes=True)
...
def model_config(input_shape):
    ...
    # compile the model
    opt = keras.optimizers.Adam(lr=0.001,name="Adam_2D")
...
def train_model(datax, datay, valx, valy):
    ...
    model.summary()
    keras.utils.plot_model(model,"../../Outputs/verification_2D_model5_ResNet34_DA_finetuning.png",show_shapes=True)
    # fit model
    history = model.fit([datax[:, 0], datax[:, 1]], datay[:], epochs=30, batch_size=64,
    ...
    opt = keras.optimizers.Adam(lr=1e-5,name="Adam_2D_2")
    model.compile(optimizer=opt, loss='binary_crossentropy',
                        metrics=['accuracy'])
    history_2=model.fit([datax[:, 0], datax[:, 1]], datay[:], epochs=15, batch_size=64,
                        validation_data=([valx[:, 0], valx[:, 1]], valy), verbose=2)
    print('Accuracy: mean=%.3f std=%.3f, n=%d' %
          (mean(history_2.history['accuracy'])*100, std(history_2.history['accuracy'])*100, len(history_2.history['accuracy'])))
    # print('Top-5 Accuracy: mean=%.3f std=%.3f, n=%d' %
    #      (mean(history.history['top_k_categorical_accuracy'])*100, std(history.history['top_k_categorical_accuracy'])*100, len(history.history['top_k_categorical_accuracy'])))
    print('Validation Accuracy: mean=%.3f std=%.3f, n=%d' %
          (mean(history_2.history['val_accuracy'])*100, std(history_2.history['val_accuracy'])*100, len(history_2.history['val_accuracy'])))
    return history,history_2, model


history,history_2, model = train_model(pairTrain, labelTrain, pairVal, labelVal)
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/verification_2D_model5_ResNet34_DA_finetuning_training_graph.png')
plt.close()
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/verification_2D_model5_ResNet34_DA_finetuning_finetuning_graph.png')
plt.close()

model.save('../Models/human_2D_verification_model5_ResNet34_DA_finetuning.h5')
print("saved")
...
plt.savefig('../../Outputs/verification_2D_model5_ResNet34_DA_finetuning_roc.png')
plt.close()
roc_display = RocCurveDisplay(fpr=fpr, tpr=tpr, roc_auc=auc)
roc_display.plot()
plt.savefig('../../Outputs/verification_2D_model5_ResNet34_DA_finetuning_display_roc.png')
plt.close()
fnr = 1 - tpr
...
plt.savefig('../../Outputs/verification_2D_model5_ResNet34_DA_finetuning_confusionMatrix.png')
plt.close()
...
legend = ax.legend()
plt.savefig('../../Outputs/verification_2D_model5_ResNet34_DA_finetuning_EER.png')
plt.close()