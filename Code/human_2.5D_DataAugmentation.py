...
def DA_handler(path,folder):
    ...
    for fn in img_names:
        ...
        crop_write = tf.write_file(folder+'/'+name.replace('.jpg', '_cropped.jpg'), cropped_img)
        sess.run(crop_write)
        ...
        for i in range(5):
            ...
            rotate_write = tf.write_file(folder+'/'+name.replace('.jpg', '_rotated_'+str(i+1)+'.jpg'), rotated_img)
        ...
        noise_write = tf.write_file(folder+'/'+name.replace('.jpg', '_noise.jpg'), noised_img)  
        sess.run(noise_write)
    return

DA_handler(all_images_cropped,'../../UND 2.5D cropped face data/2.5D_cropped_data_DA')
